...
let invalid msg value =
  invalid_arg (Printf.sprintf "%s: %s" msg value)

let numeric_type_of_string text =
  if String.length text < 2 then
    invalid "numeric type too short" text
  else
    let signedness =
    ...
    in
    let bits =
      int_of_string (String.sub text 1 (String.length text - 1))
    in
    { signedness; bits }
...
let vec_type_of_string text =
  ...
  if starts_with ~prefix text then
    let dimension =
      int_of_string (String.sub text (String.length prefix) (String.length text - String.length prefix))
    in
    { kind = FloatVec; dimension }
  else
    invalid "unsupported vector type" text

let mat_type_of_string text =
  let (kind, prefix_len) =
    if starts_with ~prefix:"fmat" text then
      (FloatMat, 4)
    else if starts_with ~prefix:"mat" text then
      (GenericMat, 3)
    else
      invalid "unsupported matrix type" text
  in
...
let decode_literal_body text quote_char =
  ...
  if len < 2 || String.get text 0 <> quote_char || String.get text (len - 1) <> quote_char then
    invalid "literal missing quotes" text;
...
let string_literal_of_lexeme text =
  decode_literal_body text '"'
...
let int_literal_of_lexeme text =
  int_of_string text

let float_literal_of_lexeme text =
