...
and loop_stmt_desc = {
  ...
  body : block;
  step : statement list;
  iteration_hint : iteration_hint option;
}

and loop_stmt = loop_stmt_desc node
...
and write_desc = { target : expression; value : expression }
and write = write_desc node
...
and parsed_program = { program : program }
